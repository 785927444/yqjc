  ...
  export default class Devices {
    ...
    async getInit(key, type) {
      let query = isNull(publicStore().distributId) || publicStore().distributId=='0'? { model: "t_sensor", args: `type='${this.type+'_node'}'`} : { model: "t_sensor", args: `stationNum='${publicStore().distributId}' and type='${this.type+'_node'}'`}
      let query1 = isNull(publicStore().distributId) || publicStore().distributId=='0'?{ model: "t_sensor", args: `type='${this.type}'`} : { model: "t_sensor", args: `stationNum='${publicStore().distributId}' and type='${this.type}'`}
      ...
      let query4 = { model: "t_station", args: `parent_id='0'`}
      let res = {}
      if(this.options && this.options.node){
        res = await publicStore().http({ Api: query, Api1: query1, Api2: query2, Api3: query3, Api4: query4})
        publicStore()._public.sensorNode = isNull(res.Api)? [] : res.Api
      }else{
        res = await publicStore().http({ Api1: query1, Api2: query2, Api3: query3, Api4: query4})
      }
      publicStore()._public.stations = isNull(res.Api4)? [] : res.Api4
      ...
      if(isNull(publicStore()._public.allDevices)){
        publicStore()._public.devices = [];
        publicStore()._public.homeInfo = {};
        publicStore()._public.homeDevices = [];
        publicStore()._public.sensorNodes = [];
        return
      }
    ...
    // 获取设备信息
    async getDevices() {
      if(isNull(publicStore().active) && publicStore().distributId!='0') return
      publicStore()._public.devices = isNull(publicStore().distributId) || publicStore().distributId=='0'? [...publicStore()._public.allDevices] : 
      publicStore().active.parent_id == '0'? publicStore()._public.allDevices.filter(a=>a.stationNum == publicStore().active.id) :
