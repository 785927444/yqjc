...
export default class DsModel {
  ...
  getBox() {
    ...
    // 包围盒辅助器
    // let boxHelper = new THREE.Box3Helper(box, 0xfff000)
   ...
   * 
   */
  updateAction(i = 0, duration=0.2) {
    if(i != this.animalIndex){
      // 取消原来动作
      this.mixer.clipAction(this.model.animations[this.animalIndex]).fadeOut(duration)
      // 执行切换动作
      this.mixer.clipAction(this.model.animations[i]).reset().fadeIn(duration).play()
      // 更新状态值
      this.animalIndex = i
    }
  }
   ...
   */
  getAnimatTime() {
    const animate = this.model.animations[this.animalIndex]
    if(animate){
  ...
  openCastShadow(names = []) {
    this.model.scene.traverse(mesh => {
      if (names.length == 0 || (mesh.type === 'Mesh' && names.indexOf(mesh.name) === -1)) {
        if(mesh.material) mesh.material.side = THREE.DoubleSide // 双面显示
