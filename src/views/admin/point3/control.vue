...
<script lang="ts" setup>
  ...
  const onVisable = async (val?: any) => {
    ...
    state.old = {...val}
    state.form = {...val}
    if(!proxy.isNull(publicStore._public.controls)){
  ...
  const handleSubmit = async(formEl) => {
    ...
    let params = {query: query, ip: proxy.getHttp('ip'), port: state.form.mode&&state.form.mode==41?"9527":"8808", url: state.form.mode&&state.form.mode==41?"/api/v1.0/p3000/monitorOnline/setparam":"/api/v1.0/p3000/monitorOnline/commFrontEndProcessor/sensor/remoteCtrl"}
    api.httpApi(params).then((res:any) => {
      ...
      if(res.code == 200 && res.data && (res.data.errcode == 0 || res.data.errcode == 1)){
        ...
        onVisable()
        // let querys = [{sensor_id: state.form.sensorid, measurement_code: state.form.code, timestamp: endTime, value: state.form.value+''}]
        // let paramss = {query: querys, ip: proxy.getHttp('ip'), port: '10132',  url: "/api/v1.0/p3000/monitorOnline/commFrontEndProcessor/UpdateSensorData/auto"}
        // publicStore.http({httpApi: paramss}).then((rex:any) => {console.log("rex")})
      }else{
