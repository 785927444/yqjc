  ...
  <div class="layout-col">
    ...
    <div class="h40 ww100 flex-sc relative mb10">
      ...
      <div class="ww100 hh100 flex-sc absolute pl35">
        <span class="fw flex1 ptb5">当日其他信息情况</span>
    ...
    <div class="layout-col p20 ba1">
      <div class="hh100 ww100" id="Line_orterData1" v-if="state.active==1"></div>
      <div class="hh100 ww100" id="Line_orterData2"  v-if="state.active==2"></div>
    </div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  watch(() => state.active, async(val, old) => {
    if(state.active == 1){
      await publicStore.rechart('Line_orterData1')
      await nextTick();
      setChart1()
    }
    if(state.active == 2){
      await publicStore.rechart('Line_orterData2')
      await nextTick();
      setChart2()
    }
  }, { immediate: false, deep: true });

  watch(() => publicStore._public.orterData1, async(val, old) => {
  ...
  watch(() => publicStore._public.orterData2, async(val, old) => {
	...
	const setChart1 = async() => {
		...
		let data = publicStore._public.orterData1
		let series = {data: []}
		series = data.map(v => {
	...
	const setChart2 = async() => {
		...
		let data = publicStore._public.orterData2
		let series = {data: []}
		series = data.map(v => {
			attr.legendArr.push(v.name)
			let data = v.data
