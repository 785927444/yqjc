  ...
  <div class="layout-col">
    ...
    <div class="h40 ww100 flex-sc relative mb10">
      ...
      <div class="ww100 hh100 flex-sc absolute pl35">
        <span class="fw flex1 ptb5">当日日充电功率预测</span>
    ...
    <div class="layout-col p20 ba1">
      <div class="hh100 ww100" id="Line_fcstData"></div>
    </div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  watch(() => publicStore._public.fcstData, async(val, old) => {
    if(state.active==2) return
    if(proxy.isNull(val)) return
    if(JSON.stringify(val) == JSON.stringify(old)) return
    await nextTick();
    setChart(publicStore._public.fcstData, "Line_fcstData", {legendShow: true, y_name: 'kw', tool_name_x: '日期：', tool_name_y: '值：'});
  }, { immediate: false, deep: true });

	const setChart = async() => {
		let chart = 'Line'
		let id = 'Line_fcstData'
		let attr = {
			min: 0, 
			// max: 100,
			extent: 4, 
		...
		let data = publicStore._public.fcstData
		let series = {data: []}
		series = data.map(v => {
