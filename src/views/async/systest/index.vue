<template>
  <div class="layout-col">
   ...
   <div class="ww100 flex-sc ba9 h40 mb15">
    <div class="cursor hh100 ptb10 plr20" v-for="(v, i) in state.selectPath" :key="i" :class="v.path==route.path?'ba10':'i21'" @click="toPath(v.path)">{{v.name}}</div>
	  <div class="flex1 hh100 flex-ec">
	  <img src="@/assets/imgs/uu1.png" />
	  <span class="ml5 cc" @click="state.prePath.path?toPath(state.prePath.path):''">{{state.prePath.name}}</span>
	  <span class="mlr10 cc">/</span>
	  <span class="mr5">{{find(state.selectPath, ['path', route.path], 'name') }}</span>
    ...
    <div class="ww100 flex-bc plr15">
      <div class="flex-sc">
        <el-button class="rad4 ptb5 plr8 flex-cc cursor bgi22 i21 ml10 bo-i21-1" @click.stop="handleClick('selectAll')" :icon="Plus">发布邀约</el-button>
      </div>
      <div class="flex-ec cursor flex1">
        <div class="ml15 mr10">项目名称</div>
        <div class="flex w50x4">
          <el-input v-model="state.project_name" placeholder="请输入" style="width: 100%" clearable filterable />
        </div>
...
<script lang="ts" setup>
  ...
  let modes = $ref([
    {value: '0', name: '边端模式', color: ''},
    {value: '1', name: '云边端模式', color: ''},
  ])
  let invi_types = $ref([
  ...
  let statuss = $ref([
    {value: '0', name: '待邀约', color: 'i21'},
    {value: '1', name: '待执行', color: 'i21'},
    {value: '2', name: '正在执行', color: 'i6'},
    {value: '3', name: '执行完成', color: 'i26'},
    {value: '4', name: '已结算', color: 'i26'},
  ])
  const state = reactive({
	  ...publicStore.$state,
    prePath: {name: '系统测试', path: ''},
    selectPath: [
      {name: '通信性能测试', path: '/systest'},
      {name: '调控性能测试', path: '/demand2'},
      {name: '规约单次测试', path: '/demand3'},
    ],
    key: 'id',
    query: {"o": "o_sensor_alarm_log"},
    content: [
      { width: 'w70',   show: true, align: 'center', key: '*', name: '序号' },
      { width: 'w50x4', show: true, align: 'left',  key: 'project_name', name: '模式' },
      { width: 'w50x3', show: true, align: 'left',  key: 'invi_type', name: '邀约类型', type: 'find', value: [invi_types, 'value', 'name'] },
      { width: 'w50x3', show: true, align: 'left',  key: 'time_type', name: '时间类型', type: 'find', value: [time_types, 'value', 'name'] },
      { width: 'w50x3', show: true, align: 'left',  key: 'cap', name: '需求容量（kw）' },
      { width: 'w50x3', show: true, align: 'left',  key: 'area', name: '需求地区' },
      { width: 'w50x4', show: true, align: 'left',  key: 'start_time', name: '响应开始时间', type: 'time' },
      { width: 'w50x4', show: true, align: 'left',  key: 'end_time', name: '响应结束时间', type: 'time' },
      { width: 'w50x4', show: true, align: 'left',  key: 'deal_time', name: '出清时间', type: 'time' },
      { width: 'w50x3', show: true, align: 'left',  key: 'status', name: '状态', type: 'find', value: [statuss, 'value', 'name'] },
      { width: 'flex1', show: true, align: 'right', key: {invi: '去邀约', view: '查看', res: '出清结果'},  name: '操作' },
    ],
    project_name: '',
    type: '',
    clickArr: [],
    list: [
      {id: '1', project_name: '项目1##', invi_type: '0', time_type: '0', cap: '10', area: '地区1##', start_time: '2024-7-22 18:00:00', end_time: '2024-7-22 18:00:00', deal_time: '2024-7-22 18:00:00', status: '0'},
      {id: '2', project_name: '项目2##', invi_type: '0', time_type: '0', cap: '10', area: '地区2##', start_time: '2024-7-22 18:00:00', end_time: '2024-7-22 18:00:00', deal_time: '2024-7-22 18:00:00', status: '0'},
    ]
  })
  let exposeRef = $ref()
  let alarmRef = $ref()
