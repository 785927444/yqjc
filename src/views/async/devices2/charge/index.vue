...
<script lang="ts" setup>
  ...
  onMounted(async () => {
    state.devices = new Devices({type: 'charge', node: true})
    createScheduled("charge_control", 5 * 1000, () => {
      init(true)
    });
  });
  ...
  const init = async(key) => {
    ...
    state.devices.getDevicesBar('DevicesDailyInput', [{name: '日充电量', code: 'Daily_Input', data: []}], '1');
    state.devices.getDevicesLine('DevicesInputPower', [{name: '充电功率', code: 'Input_Power', data: []}]);
  }
