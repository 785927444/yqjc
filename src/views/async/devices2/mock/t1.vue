<template>
  <div class="layout-col ba1 p10">
    <div class="ww100 flex-bc plr20 bs" style="border-bottom: none;">  
      <span class="flex-sc h42 f16">光伏发电量趋势图</span>
      <span class="flex-sc ml20"></span>
    </div>
    <!-- 内容 -->
    <div class="layout-col p20 bs">
      <div class="hh100 ww100" id="Bars_DevicesDailyOutput1"></div>
    </div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  watch(() => publicStore._public.DevicesDailyOutput1, async(val, old) => {
  ...
  const setChart = async(data) => {
    let chart = 'Bars'
    let id = 'Bars_DevicesDailyOutput1'
