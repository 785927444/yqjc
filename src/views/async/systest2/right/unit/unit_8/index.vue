<template>
	<div class="layout-row">
		<div class="hh100 flex1 flex-col hidden">
			<l1 class="ww100 flex2 flex-col" @init="init" @getChild="getChild" v-model:type="state.type" />
...
<script lang="ts" setup>
  import l1 from '../../L1'
	...
	const state = reactive({
		...publicStore.$state,
		model: 't_test_task',
		type: 'airconditionerfz',
	})

	watch(() => publicStore.active, async(val, old) => {
		if(proxy.isNull(val)) return
		if(!proxy.isNull(old) && old.id == val.id) return
		console.log("监控unit_8")
    init(true)
	}, { immediate: false, deep: true });

  onMounted(async() => {
		console.log("初始化unit_8")
    init(true)
  })

	const init = async(status?: any) => { getChild(status, state.type) }
