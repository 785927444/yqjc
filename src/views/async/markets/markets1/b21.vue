<template>
  <div class="layout-col" style="overflow:visible;">
    ...
    <div class="hh100 ww100" id="Line_stockEnv1"></div>
    <!-- 导入 -->
...
<script lang="ts" setup>
  ...
  const state = reactive({
	...
	// 父子双向绑定
	const props = defineProps(['query', 'active'])
	// 父子绑定方法
  ...
  watch(() => props.active, async(val,) => {
    ...
    state.model = `t_medium_yide${val}`
  }, { immediate: false, deep: true });

  watch(() => publicStore._public.stockEnv1, async(val, old) => {
    // if(proxy.isNull(val)) return
  ...
  const handleClick = async(remark) => {
    state.model = `t_medium_yide${props.active}`
    if(remark == 'out'){
      let query = {model: `t_medium_yide${props.active}`}
      let res = await publicStore.http({Api: query})
      res.forEach(v => {
        if(v.datetime && v.datetime.indexOf('+')!=-1) v.datetime = v.datetime.split('+')[0];
      })
      let list = [...res]
      let name = `现货市场行情-曲线模板`
      let content = [
        {key: 'id', name: 'ID'},
        ...
        {key: 'powerNum', name: '用电量'},
        {key: 'currentPrice', name: '日前价格'},
        {key: 'realTimePrice', name: '实时价格'},
      ]
