        ...
        <div class="ww100 flex-sc relative mt10">
          ...
          <div class="flex1 pr10">
            <div class="ww100 flex-sc bs4 p20">
              ...
              <span class="ml15">响应总价格</span>
              <span class="flex1 tr fw f20">{{ !isNull(state.data)&&!isNull(state.data.key3)?state.data.key3:'0' }}<span class="ml8 f12 i15">元/kW</span></span>
            </div>
          </div>
          <div class="flex1 pr10">
            <div class="ww100 flex-sc bs4 p20">
              <img class="w50" src="@/assets/imgs/zz2.png" />
              <span class="ml15">响应总容量</span>
              <span class="flex1 tr fw f20">{{ !isNull(state.data)&&!isNull(state.data.key5)?state.data.key5:'0' }}<span class="ml8 f12 i15">kW</span></span>
            </div>
          </div>
        </div>
        <!-- 客户列表 -->
        <div class="layout-col">
          ...
          <div class="ww100 flex-bc ptb20">
            ...
            <div class="flex-ec cursor flex1">
              ...
              <el-button class="rad4 ptb5 plr10 flex-cc cursor bgi22 i21 ml10 bo-i21-1" @click.stop="handleClick('executes')" :icon="Edit">批量执行</el-button>
            </div>
          </div>
          <!-- 内容 -->
          <div class="layout-col">
          ...
          <!-- 底部 -->
          <div class="ww100 plr20 ptb10 bgi22 flex-ec" v-if="state.form.status == '2'">
            <div class="rad4 ptb6 plr10 flex-col-cc cursor bgi1 white ml10 bo-i21-1" @click.stop="strat()">开始执行</div>
          </div>
        </div>
      </div>
      <!--  客户详情 -->
      <executeDetail @init="init" ref="executeDetailRef" />
    </el-dialog>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const statuss = [
    {value: '2', name: '待开始', color: 'i6'},
    {value: '3', name: '已开始', color: 'i11'},
  ]
  const state = reactive({
    ...
    content: [
      ...
      { width: 'w50x2', show: true, align: 'left',  key: 'deal_capacity', name: '响应容量' },
      ...
      { width: 'w50x2', show: true, align: 'left',  key: 'status', name: '客户状态', type: 'select', list: statuss, value: 'value', label: 'name' },
      { width: 'w50x4', show: true, align: 'left',  key: 'execute_time', name: '开始时间', type: 'time' },
      { width: 'w50x4', show: true, align: 'left',  key: 'work_time', name: '工作时长' },
      { width: 'w50x2', show: true, align: 'left',  key: 'work_price', name: '工作金额' },
      { width: 'flex1', show: true, align: 'right', key: {execute: '开始', detail: '详情'},  name: '操作' },
    ],
    click: false,
    clickArr: [],
    name: '',
  ...
  const setInit = async() => {
    let query = {model: 't_invit', args: `task_id='${state.form.id}' and status>='2' and confirm='1'`} 
    let res = await publicStore.http({Api: query})
    let list1 = proxy.isNull(res)? [] : res
    let list2 = list1.filter(a=>a.status == '3')
    state.data.key1 = list1.length
    state.data.key2 = list2.length
    if(!proxy.isNull(list1)){
      const prices = list1.map(a => parseFloat(a.deal_price))
      state.data.key3 = Math.max(...prices)
      state.data.key4 =  Math.min(...prices)
    }
    if(!proxy.isNull(list2)){
      state.data.key5 = list2.reduce((sum, item) => sum + Number(item.deal_capacity), 0);
    }
  }

	const init = async() => {
    setInit()
    let args = `confirm='1' and `
    ...
    let field = 'b.*, IFNULL(i.status, 0) AS status, i.id AS invit_id, i.deal_price, i.deal_capacity, i.deal_devices, i.deal_start, i.deal_end, i.deal_time, i.confirm, i.confirm_text, i.execute_time, i.work_time'
  ...
  const handleClick = (remark, val) => {
    if(remark == 'detail') {
      console.log("val", val)
      executeDetailRef.onVisable(val)
    }
    if(remark == 'active'){
    ...
    if(remark == 'execute') {
      if (val.status == '3') return ElNotification({ title: '提示', message: '已开始，请重新选择！', type: 'error' })
      strat([val])
    }
    if(remark == 'executes') {
      let tempval = state.clickArr.find(a=>a.status != '3')
      ...
      strat(state.clickArr)
    }
  }

  const strat = async(list) => {
    let time = new Date().getTime()
    if(time < state.form.start_time) return ElNotification({ title: '提示', message: '开始失败，当前时间必须大于开始时间', type: 'error' })
    if(time > state.form.end_time) return ElNotification({ title: '提示', message: '开始失败，当前时间必须大于结束时间', type: 'error' })
    ElMessageBox.confirm('是否确定开始?', '温馨提示', {confirmButtonText: '确定', cancelButtonText: '关闭', type: 'success'}).then(() => {
      let params = {model: 't_invit', list: []}
      params.list = list.map(v => {
        let temp = {id: v.invit_id, execute_time: time}
        if(v.status == '2') temp.status = '3'
        return temp
      })
