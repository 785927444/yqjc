        ...
        <div class="layout-col">
          ...
          <div class="ww100 flex-bc ptb20">
            ...
            <div class="flex-ec cursor flex1">
              ...
              <div class="ml15 mr5">反馈状态：</div>
              <div class="flex w50x4">
                <el-select v-model="state.confirm" placeholder="请选择" style="width: 100%" clearable filterable @change="state.click=false;state.clickArr=[]">
                  <el-option label="全部" value="-1" />
                  <el-option v-for="(v, i) in confirmss" :key="v.value" :label="v.name" :value="v.value" />
                </el-select>
              </div>
...
<script lang="ts" setup>
  ...
  const state = reactive({
    ...
    content: [
      ...
      { width: 'w50x4', show: true, align: 'left',  key: 'confirm_text', name: '反馈消息' },
      { width: 'flex1', show: true, align: 'right', key: {confirm: '确认', detail: '详情'},  name: '操作' },
    ],
    click: false,
    clickArr: [],
    name: '',
  ...
  const setInit = async() => {
    let query = {model: 't_invit', args: `task_id='${state.form.id}' and status>='2'`} 
    let res = await publicStore.http({Api: query})
    let list1 = proxy.isNull(res)? [] : res
    let list2 = list1.filter(a=>a.confirm == '1')
  ...
  const strat = async() => {
    ...
    ElMessageBox.confirm('是否确定开始?', '温馨提示', {confirmButtonText: '确定', cancelButtonText: '关闭', type: 'success'}).then(() => {
      if(state.form.status == '2') setTaskStatus('3')
    })
  }
  ...
  const setTaskStatus = async(status) => {
    ...
    api.updApi(params).then((res:any) => {
      if(res.code == 200){
        init()
        ElNotification({ title: '提示', message: '开始成功，请通知客户', type: 'success' })
      }else{
