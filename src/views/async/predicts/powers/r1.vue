  ...
  <div class="layout-col">
    ...
    <div class="h40 ww100 flex-sc relative mb10">
      ...
      <div class="ww100 hh100 flex-sc absolute pl35">
        <span class="fw flex1 ptb5">发电曲线</span>
        <div class="flex-ec flex1 hh100">
          <div class="ml15">用户：</div>
          <div class="w100x4">
            <el-select v-model="state.userid" placeholder="请选择" style="width:100%" clearable filterable :multiple="true" @change="publicStore.rechart('Line_powersReal');changeUserid()">
    ...
    <div class="layout-col p20 ba1">
      ...
      <div class="flex1 ww100 hidden flex-col mb20">
        <div class="ww100 flex-bc h40 lh40 bgi22 i15 mb15">  
          ...
          <span class="flex-sc pr10 cursor" @click="state.check = !state.check; publicStore.rechart('Line_powersReal'); setChart1(publicStore._public.powersReal)">
        ...
        <div class="hh100 ww100" id="Line_powersReal"></div>
      </div>
      <!-- 预测 -->
       <div class="flex1 ww100 hidden flex-col">
        ...
        <div class="hh100 ww100" id="Line_powersPredict1" v-if="state.active==1"></div>
        <div class="hh100 ww100" id="Line_powersPredict2" v-if="state.active==2"></div>
       </div>
    </div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  watch(() => state.active, async(val, old) => {
    await nextTick();
    if(state.active == 1){
      await publicStore.rechart('Line_powersPredict1')
      setChart3(publicStore._public.powersPredict1);
    }
    if(state.active == 2){
			await publicStore.rechart('Line_powersPredict2')
			setChart4(publicStore._public.powersPredict2);
    }
  }, { immediate: false, deep: true });

  watch(() => publicStore._public.powersReal, async(val, old) => {
  ...
  watch(() => publicStore._public.powersPredict1, async(val, old) => {
  ...
  watch(() => publicStore._public.powersPredict2, async(val, old) => {
    if(state.active==2) return
    if(proxy.isNull(val)) return
    if(JSON.stringify(val) == JSON.stringify(old)) return
    await nextTick();
  ...
  const changeUserid = async() => {
    setChart1(publicStore._public.powersReal)
    if(state.active == 1){
      setChart3(publicStore._public.powersPredict1)
    }
    if(state.active == 2){
