...
<script lang="ts" setup>
  ...
  const mock = async() => {
    ...
    let throughput = []
    let datas = []
    let childs = JSON.parse(JSON.stringify(publicStore.active.attr.childs))
    const now = Date.now()
    for (let i = 0; i < count; i++) {
      ...
      childs.forEach(v => {
        v.items.forEach(vv => {
          let temp = { act_value: Math.floor(Math.random() * 10) + 1, act_res: "1", act_error: i, act_reason: `原因${i}`, throughput: [...throughput]}
          Object.assign(vv, temp)
        })
      })
      let item = {value: {childs: JSON.parse(JSON.stringify(childs))}, time: frequency*1000}
      datas.push(item)
    } 
